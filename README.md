...
- 비교: 현재 폐업률 기준선·일반 선형회귀·Ridge 회귀를 같은 홀드아웃에서 비교
- 반복 검증: 과거 시점부터 순차적으로 학습 구간을 확장하는 롤링 시간 검증으로 분기별 MAE·RMSE 평균과 변동성을 확인
- 업종별 공개: 홀드아웃 오차를 업종별로 함께 산출해 특정 업종의 약한 성능을 숨기지 않음
...
데이터 재생성 전에는 점포·매출·유동인구 음수 여부, 개·폐업률 범위, 프랜차이즈 수와 전체 점포 수의 관계를 자동 검사합니다. 하나라도 위반하면 결과 JSON을 만들지 않고 오류를 내므로 잘못된 원천 파일이 배포 데이터로 들어가는 것을 막습니다.
...
- `data_quality_checks.csv`: 키 중복·비정상 비율·음수 값 등 배포 차단용 데이터 계약 검사
...
- `model_validation_summary.csv`: 기준선·일반 선형회귀·Ridge의 홀드아웃 및 롤링 검증 비교
- `industry_holdout_metrics.csv`: 업종별 홀드아웃 MAE·RMSE
- `startup_fit_sensitivity.csv`: 안정성 우선·성장 우선 가중치에서의 추천 순위 민감도
...
창업 적합도는 성공 확률이 아니라 **설명 가능한 다기준 의사결정 점수**입니다. 기본 가중치와 안정성 우선·성장 우선 가중치에서 동일 업종 후보 순위가 얼마나 변하는지 분석해, 특정 가중치에 결과가 과도하게 의존하는지 확인합니다.
...
GenAI 응답은 허용된 근거 항목, 항목 수, 길이, 과도한 확정 표현을 서버에서 다시 검사합니다. 검사에 실패한 응답은 사용자에게 노출하지 않고 규칙 기반 코치로 전환합니다. `tests/test_coach_guardrails.py`는 근거 없는 항목과 성공 단정 표현이 거부되는지 확인하는 재현 가능한 평가 케이스입니다.
